...
const IcGear = () => (
  <svg
    ...
    style={{
      flexShrink: 0,
      minWidth: 16, // 🔥 크롬에서 가로 비율이 찌그러지는 것을 완벽 차단
      display: "block",
    }}
  >
...
function ManageCatsModal({ cats, onClose, db }) {
  ...
  const moveCat = async (ci, dir) => {
    ...
    newCats[ci + dir] = temp;
    await updateDoc(doc(db, "cats_data", String(newCats[ci].id)), {
      order: ci,
    });
    await updateDoc(doc(db, "cats_data", String(newCats[ci + dir].id)), {
      order: ci + dir,
    });
  };
              ...
              <div style={{ display: "flex", gap: 4, marginLeft: "auto" }}>
                ...
                <button
                  ...
                  style={{
                    ...iBtn(),
                    color: C.muted,
                    visibility: ci < cats.length - 1 ? "visible" : "hidden",
                  }}
                >
                  <IcDown />
                </button>
                  ...
                  <div style={{ display: "flex", gap: 4, marginLeft: "auto" }}>
                    ...
                    <button
                      ...
                      style={{
                        ...
                        color: C.muted,
                        visibility:
                          si < cat.sub.length - 1 ? "visible" : "hidden",
                      }}
                    >
                      <IcDown />
                    </button>
...
export default function App() {
  ...
  useEffect(() => {
    const unsubCats = onSnapshot(collection(db, "cats_data"), (snap) => {
      ...
      if (data.length === 0) {
        DEF_CATS.forEach((c) =>
          setDoc(doc(db, "cats_data", String(c.id)), { ...c, isDeleted: false })
        );
      } else {
        setCats(
          data
    ...
    const unsubRests = onSnapshot(collection(db, "rests_data"), (snap) => {
      ...
      if (data.length === 0) {
        DEF_RESTS.forEach((r) =>
          setDoc(doc(db, "rests_data", String(r.id)), {
            ...r,
            isDeleted: false,
          })
        );
  ...
  const saveRest = async () => {
    ...
    if (md?.id) {
      ...
      await addDoc(collection(db, "rests_data"), {
        ...finalForm,
        reviews: [],
        isDeleted: false,
      });
  ...
  const restoreCat = async (id) =>
    await updateDoc(doc(db, "cats_data", String(id)), { isDeleted: false });
  const hardDelCat = async (id) => {
    if (window.confirm("카테고리를 영구 삭제하시겠습니까? 복구할 수 없습니다."))
      await deleteDoc(doc(db, "cats_data", String(id)));
  };
  const restoreRest = async (id) =>
    await updateDoc(doc(db, "rests_data", String(id)), { isDeleted: false });
  const hardDelRest = async (id) => {
    if (window.confirm("맛집을 영구 삭제하시겠습니까? 복구할 수 없습니다."))
      await deleteDoc(doc(db, "rests_data", String(id)));
  };
  ...
  const saveReview = async () => {
    ...
    if (editingRevId) {
      updatedReviews = selected.reviews.map((rv) =>
        rv.id === editingRevId
          ? { ...rv, text, author, rating: revForm.rating }
          : rv
      );
    } else {
      updatedReviews = [
        ...(selected.reviews || []),
        { id: Date.now(), text, author, rating: revForm.rating },
      ];
    }
    await updateDoc(doc(db, "rests_data", String(selId)), {
  ...
  const delReview = async (rid, revId) => {
    ...
    await updateDoc(doc(db, "rests_data", String(rid)), {
  ...
  return (
    <div>
      ...
      <div className="main-grid">
        ...
        >
          <div style={{ marginTop: -24, marginBottom: 36 }}>
            ...
            <div style={{ fontSize: 13.5, fontWeight: 200, color: C.gray }}>
              팔로미노 맛집 아카이브
            </div>
          </div>
          ...
          <div>
            ...
            >
              <span style={{ fontSize: 13.5, fontWeight: 300, color: C.ink }}>
                카테고리
              </span>
              <button
            ...
            >
              <button
                ...
                style={{
                  ...
                  borderRadius: R,
                  border: "none",
                  background: selCat === "trash" ? "#fce8e8" : "transparent",
                  cursor: "pointer",
                  gap: 8,
                  transition: "background .15s",
                }}
              >
                ...
                <span
                  style={{
                    flex: 1,
                    textAlign: "left",
                    fontWeight: selCat === "trash" ? 600 : 400,
                    fontSize: 13.5,
                    color: selCat === "trash" ? "#e44" : C.ink,
                  }}
                >
                  휴지통
                </span>
                ...
                <>
                  ...
                  >
                    <button
                      onClick={() => {
                        if (sortConfig.key === "newest") {
                          setSortConfig({
                            key: "newest",
                            dir: sortConfig.dir === "desc" ? "asc" : "desc",
                          });
                        } else {
                    ...
                    <button
                      ...
                      style={{
                        background:
                          sortConfig.key === "name" ? C.lightbg : "transparent",
                        border: "none",
                        borderRadius: R - 4,
                        padding: "0 10px",
                        height: 26,
              ...
              <>
                {trashCats.length === 0 && trashRests.length === 0 && (
                  ...
                  >
                    ...
                    <div style={{ fontSize: 13 }}>휴지통이 비어있어요</div>
                  </div>
                )}
                {trashCats.map((cat) => (
                  <div
                    ...
                    style={{
                      ...
                      padding: "12px",
                      borderRadius: R,
                      borderBottom: `1px solid ${C.lightbg}`,
                    }}
                  >
                    <div
                      style={{
                  ...
                  {!selected.reviews || selected.reviews.length === 0 ? (
                    ...
                    selected.reviews.map((rv, i) => (
                      <div
                        ...
                        style={{
                          background:
                            editingRevId === rv.id ? "#f8ffd8" : "#fff",
                          border: `1px solid ${
                            editingRevId === rv.id ? C.point : C.border
                          }`,
                          borderRadius: R,
                          padding: "11px 14px",
                          transition: "background 0.2s",
                ...
                >
                  ...
                  >
                    <div
                      style={{
                        fontSize: 12,
                        fontWeight: 500,
                        color: editingRevId ? C.ink : C.muted,
                      }}
                    >
                      {editingRevId ? " 리뷰 수정하기" : "리뷰 남기기"}
                    </div>
                    {editingRevId && (
                      <button
                        onClick={() => {
                  ...
                  <div style={{ display: "flex", gap: 7 }}>
                    <textarea
                      ...
                      onKeyDown={(e) =>
                        e.key === "Enter" && e.ctrlKey && saveReview()
                      }
                      rows={3}
                      style={{
                        ...inp(),
                    ...
                    <button
                      ...
                      style={{
                        ...
                        alignItems: "center",
                        justifyContent: "center",
                        background: editingRevId ? C.point : C.ink,
                        color: editingRevId ? C.ink : "#fff",
                        border: "none",
                        borderRadius: R,
                        padding: "0 14px",
                        fontWeight: 600,
      ...
      {modal === "manageCats" && (
        <ManageCatsModal cats={cats} onClose={() => setModal(null)} db={db} />
      )}

      {(modal === "addRest" || modal === "editRest") && (
        <Modal
          title={modal === "addRest" ? " 맛집 추가" : " 맛집 수정"}
          onClose={() => {
            setModal(null);
            setMd(null);
          }}
        >
          ...
          {/* 🔥 카테고리 선택 시 하위 카테고리가 '선택 안함(빈 값)'으로 초기화되도록 수정됨 */}
          <div style={{ marginBottom: 13 }}>
            <div style={fieldLbl}>카테고리</div>
            <select
              value={rForm.cat}
              onChange={(e) => {
                setRForm((p) => ({ ...p, cat: e.target.value, sub: "" }));
              }}
              style={inp()}
            >
              {cats.map((c) => (
