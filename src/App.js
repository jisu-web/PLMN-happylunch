...
/* ─── 컬러 토큰 & 아이콘 등 ─── */
const C = { bg: "#f7f7f7", card: "#ffffff", ink: "#181c22", gray: "#b8b8b9", lightbg: "#f7f7f7", point: "#e7f63c", border: "#e2e2e2", muted: "#b8b8b9" };
const R = 10;
...
function Stars({ value, onRate, size = 13 }) {
  ...
  return (
    <span style={{ display: "inline-flex", gap: 2 }}>
      {[1, 2, 3, 4, 5].map((i) => (
        <svg key={i} width={size} height={size} viewBox="0 0 24 24" fill={i <= n ? C.point : "none"} stroke={i <= n ? C.point : C.gray} strokeWidth="1.8" style={{ cursor: onRate ? "pointer" : "default", display: "block", flexShrink: 0 }} onMouseEnter={() => onRate && setHov(i)} onMouseLeave={() => onRate && setHov(0)} onClick={() => onRate?.(i)}>
...
const IcMap = () => <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2"><path d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7z" /><circle cx="12" cy="9" r="2.5" /></svg>;
const IcEdit = () => <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7" /><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z" /></svg>;
const IcTrash = () => <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><polyline points="3 6 5 6 21 6" /><path d="M19 6l-1.5 13.5a1.5 1.5 0 0 1-1.5 1.5H8a1.5 1.5 0 0 1-1.5-1.5L5 6" /><path d="M10 11v6M14 11v6" /><path d="M9 6V4a1 1 0 0 1 1-1h4a1 1 0 0 1 1 1v2" /></svg>;
const IcX = () => <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.4"><line x1="18" y1="6" x2="6" y2="18" /><line x1="6" y1="6" x2="18" y2="18" /></svg>;
const IcPlus = () => <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.4"><line x1="12" y1="5" x2="12" y2="19" /><line x1="5" y1="12" x2="19" y2="12" /></svg>;
const IcChev = ({ open }) => <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" style={{ transform: open ? "rotate(180deg)" : "none", transition: "transform .2s", display: "block" }}><polyline points="6 9 12 15 18 9" /></svg>;
const IcGear = () => <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"><circle cx="12" cy="12" r="3" /><path d="M19.4 15a1.65 1.65 0 0 0 .33 1.82l.06.06a2 2 0 0 1-2.83 2.83l-.06-.06a1.65 1.65 0 0 0-1.82-.33 1.65 1.65 0 0 0-1 1.51V21a2 2 0 0 1-4 0v-.09A1.65 1.65 0 0 0 9 19.4a1.65 1.65 0 0 0-1.82.33l-.06.06a2 2 0 0 1-2.83-2.83l.06-.06A1.65 1.65 0 0 0 4.68 15a1.65 1.65 0 0 0-1.51-1H3a2 2 0 0 1 0-4h.09A1.65 1.65 0 0 0 4.6 9a1.65 1.65 0 0 0-.33-1.82l-.06-.06a2 2 0 0 1 2.83-2.83l.06.06A1.65 1.65 0 0 0 9 4.68a1.65 1.65 0 0 0 1-1.51V3a2 2 0 0 1 4 0v.09a1.65 1.65 0 0 0 1 1.51 1.65 1.65 0 0 0 1.82-.33l.06-.06a2 2 0 0 1 2.83 2.83l-.06.06A1.65 1.65 0 0 0 19.4 9a1.65 1.65 0 0 0 1.51 1H21a2 2 0 0 1 0 4h-.09a1.65 1.65 0 0 0-1.51 1z" /></svg>;

const iBtn = (active) => ({ width: 32, height: 32, borderRadius: R, border: "none", background: active ? C.point : "transparent", color: C.ink, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", transition: "background .15s", flexShrink: 0 });
const fieldLbl = { fontSize: 11, fontWeight: 600, color: C.muted, textTransform: "uppercase", letterSpacing: 0.5, marginBottom: 5 };
const inp = (extra = {}) => ({ border: `1.5px solid ${C.border}`, borderRadius: R, padding: "9px 13px", fontSize: 13.5, outline: "none", fontFamily: "inherit", background: C.card, color: C.ink, width: "100%", ...extra });

function Modal({ title, onClose, width = 480, children }) {
  return (
    <div onClick={(e) => e.target === e.currentTarget && onClose()} style={{ position: "fixed", inset: 0, background: "rgba(24,28,34,.35)", zIndex: 500, display: "flex", alignItems: "center", justifyContent: "center" }}>
      <div style={{ background: C.card, borderRadius: R, padding: "28px 30px", width, maxHeight: "88vh", overflowY: "auto", boxShadow: "0 20px 60px rgba(0,0,0,.14)", animation: "fu .2s ease" }}>
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      particles.forEach((p) => {
        ...
        if (p.life > 0) {
          alive = true; ctx.globalAlpha = p.life; ctx.fillStyle = p.color;
...
function CatSideItem({ cat, rests, selCat, selSub, onCat, onSub }) {
  ...
  return (
    <div style={{ marginBottom: 1 }}>
      <button onClick={() => { onCat(cat.id); if (cat.sub?.length) setOpen((o) => !o); }} style={{ display: "flex", alignItems: "center", width: "100%", padding: "6px 10px", borderRadius: R, border: "none", background: isActive && selSub === "all" ? "#e8e8e8" : "transparent", cursor: "pointer", gap: 8, transition: "background .15s", fontFamily: "inherit" }}>
        <span style={{ fontSize: 16 }}>{cat.emoji}</span>
        <span style={{ flex: 1, textAlign: "left", fontWeight: isActive ? 600 : 400, fontSize: 13.5, color: C.ink }}>{cat.name}</span>
        <span style={{ fontSize: 11.5, color: isActive && selSub === "all" ? C.ink : C.muted, marginRight: cat.sub?.length ? 4 : 0 }}>{count}</span>
        {cat.sub?.length > 0 && <span style={{ color: C.muted }}><IcChev open={open} /></span>}
      </button>

      {open && cat.sub?.map((sub) => {
        const subActive = isActive && selSub === sub;
        const sc = rests?.filter((r) => r.cat === cat.id && r.sub === sub).length || 0;
        return (
          <button key={sub} onClick={() => { onCat(cat.id); onSub(sub); }} style={{ display: "flex", alignItems: "center", width: "100%", padding: "6px 10px 6px 36px", borderRadius: R, border: "none", background: subActive ? "#f0f0ee" : "transparent", cursor: "pointer", fontFamily: "inherit", transition: "background .15s" }}>
...
function ManageCatsModal({ cats, onClose }) {
  ...
  const delSub = async (catId, idx) => {
    ...
    await updateDoc(doc(db, "categories", catId), { sub: cat.sub.filter((_, i) => i !== idx) });
  };

  const addCat = async () => {
    if (!newName.trim()) return;
    await addDoc(collection(db, "categories"), { name: newName.trim(), emoji: selEmoji, sub: [], order: cats.length });
  ...
  const addSub = async (catId) => {
    if (!newName.trim()) return;
    const cat = cats.find(c => c.id === catId);
    await updateDoc(doc(db, "categories", catId), { sub: [...(cat.sub || []), newName.trim()] });
    setNewName(""); setAddingSubFor(null);
  };
  ...
  return (
    <Modal title="카테고리 관리" onClose={onClose} width={520}>
      <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          ...
          <div key={cat.id} style={{ border: `1.5px solid ${C.border}`, borderRadius: R, overflow: "hidden" }}>
            <div style={{ display: "flex", alignItems: "center", padding: "10px 12px", background: "#fafafa", gap: 8 }}>
              ...
              {editing?.type === "cat" && editing.catId === cat.id ? (
                <input autoFocus value={newName} onChange={(e) => setNewName(e.target.value)} onKeyDown={(e) => e.key === "Enter" && saveEditName()} style={{ ...inp(), flex: 1, padding: "4px 8px" }} />
              ) : (
                <span style={{ flex: 1, fontWeight: 600, fontSize: 14 }}>{cat.name}</span>
              )}
              <div style={{ display: "flex", gap: 4 }}>
                <button onClick={() => { setEditing({ type: "cat", catId: cat.id }); setNewName(cat.name); }} style={{ ...iBtn(), color: C.muted }}><IcEdit /></button>
              ...
              {cat.sub?.map((sub, si) => (
                <div key={si} style={{ display: "flex", alignItems: "center", padding: "5px 0", gap: 6 }}>
                  {editing?.type === "sub" && editing.catId === cat.id && editing.subIdx === si ? (
                    <input autoFocus value={newName} onChange={(e) => setNewName(e.target.value)} onKeyDown={(e) => e.key === "Enter" && saveEditName()} style={{ ...inp(), flex: 1, padding: "4px 8px", fontSize: 13 }} />
                  ) : (
                    <span style={{ flex: 1, fontSize: 13 }}>{sub}</span>
                  )}
                  <button onClick={() => { setEditing({ type: "sub", catId: cat.id, subIdx: si }); setNewName(sub); }} style={{ ...iBtn(), width: 26, height: 26, color: C.muted }}><IcEdit /></button>
                  <button onClick={() => delSub(cat.id, si)} style={{ ...iBtn(), width: 26, height: 26, color: C.muted }}><IcTrash /></button>
                </div>
              ))}
              {addingSubFor === cat.id ? (
                <div style={{ display: "flex", gap: 6, marginTop: 6 }}>
                  <input autoFocus placeholder="하위 카테고리명" value={newName} onChange={(e) => setNewName(e.target.value)} onKeyDown={(e) => e.key === "Enter" && addSub(cat.id)} style={{ ...inp(), flex: 1, fontSize: 13 }} />
                  <button onClick={() => addSub(cat.id)} style={{ background: C.ink, color: "#fff", border: "none", borderRadius: R, padding: "0 12px", fontWeight: 600, fontSize: 13 }}>추가</button>
                  <button onClick={() => { setAddingSubFor(null); setNewName(""); }} style={{ background: C.lightbg, border: "none", borderRadius: R, padding: "0 10px", color: C.muted }}>취소</button>
                </div>
              ) : (
                <button onClick={() => { setAddingSubFor(cat.id); setNewName(""); }} style={{ marginTop: 6, display: "flex", alignItems: "center", gap: 5, background: "none", border: `1.5px dashed ${C.border}`, borderRadius: R, padding: "5px 10px", color: C.muted, fontSize: 12.5 }}><IcPlus /> 하위 추가</button>
              )}
            </div>
          </div>
        ))}
        ...
        {addingCat ? (
          <div style={{ border: `1.5px solid ${C.point}`, borderRadius: R, padding: "14px" }}>
            <div style={{ display: "flex", flexWrap: "wrap", gap: 5, marginBottom: 10 }}>
              {EMOJIS.map((em) => (
                <button key={em} onClick={() => setSelEmoji(em)} style={{ fontSize: 18, background: selEmoji === em ? "#f0ffa0" : C.lightbg, border: `2px solid ${selEmoji === em ? C.point : "transparent"}`, borderRadius: R, padding: "4px 6px" }}>{em}</button>
              ))}
            </div>
            <div style={{ display: "flex", gap: 7 }}>
              <input autoFocus placeholder="카테고리 이름" value={newName} onChange={(e) => setNewName(e.target.value)} onKeyDown={(e) => e.key === "Enter" && addCat()} style={{ ...inp(), flex: 1 }} />
              <button onClick={addCat} style={{ background: C.ink, color: "#fff", border: "none", borderRadius: R, padding: "0 14px", fontWeight: 600 }}>추가</button>
              <button onClick={() => { setAddingCat(false); setNewName(""); }} style={{ background: C.lightbg, border: "none", borderRadius: R, padding: "0 12px", color: C.muted }}>취소</button>
            </div>
          </div>
        ) : (
          <button onClick={() => { setAddingCat(true); setNewName(""); setSelEmoji("🍽️"); }} style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 6, padding: "11px", background: C.card, border: `1.5px dashed ${C.border}`, borderRadius: R, cursor: "pointer", color: C.muted }}><IcPlus /> 카테고리 추가</button>
        )}
      </div>
    </Modal>
  );
...
export default function App() {
  ...
  const [editingRevId, setEditingRevId] = useState(null); // 수정 중인 리뷰 아이디
  ...
  const doRandom = (type) => {
    ...
    const iv = setInterval(() => {
      ...
      if (++c > 16) {
        clearInterval(iv); setSpinning(false); setPickAnim(true);
        setConfettiKey((k) => k + 1); setConfettiActive(true);
  ...
  const saveReview = async () => {
    ...
    if (editingRevId) {
      // 수정 모드일 때
      updatedReviews = selected.reviews.map(rv => 
        rv.id === editingRevId ? { ...rv, text: revForm.text.trim(), author, rating: revForm.rating } : rv
      );
    } else {
      // 추가 모드일 때
      const newReview = { id: Date.now(), text: revForm.text.trim(), author, rating: revForm.rating };
      updatedReviews = [...(selected.reviews || []), newReview];
    }
  ...
  const listTitle = selCat === "all" ? "전체 맛집" : selSub !== "all" ? `${catOf(selCat)?.name} · ${selSub}` : catOf(selCat)?.name || "맛집";
  ...
  return (
    <div style={{ minHeight: "100vh", background: C.bg, fontFamily: "'Pretendard', sans-serif" }}>
      <style>{`
      ...
      <div className="main-grid">
        {/* 왼쪽 패널 */}
        <div className="left-panel" style={{ position: "sticky", top: 40, display: "flex", flexDirection: "column" }}>
          <div style={{ marginBottom: 36 }}>
            <h1 style={{ fontSize: 40, fontWeight: 200, color: C.ink, letterSpacing: -1, lineHeight: 1.2, marginBottom: 8 }}>점심<br />뭐 먹지?</h1>
            <div style={{ fontSize: 13, fontWeight: 200, color: C.gray }}>팔로미노 맛집 아카이브</div>
          </div>

          <div style={{ marginBottom: 20 }}>
            <span style={{ fontSize: 13, color: C.ink, display: "block", marginBottom: 10 }}>랜덤 추천</span>
                ...
                <div style={{ borderRadius: R, padding: "12px 14px", background: C.card, border: `1.5px solid ${pickAnim ? C.point : C.border}`, animation: pickAnim ? "pi .3s ease" : "none", position: "absolute", inset: 0, display: "flex", flexDirection: "column", justifyContent: "center", cursor: pickType === "rest" ? "pointer" : "default" }} onClick={() => pickType === "rest" && setSelId(pick.id)}>
                  ...
                  {pickType === "rest" ? (
                    ...
                    <>
                      ...
                      <div style={{ fontWeight: 600, fontSize: 14, color: C.ink }}>{pick.name}</div>
                      <div style={{ fontSize: 12, color: C.muted, marginTop: 2 }}>{rests?.filter((r) => r.cat === pick.id).length || 0}개 맛집</div>
                    </>
                  )}
                </div>
              ) : (
          ...
          <div>
            ...
            <button onClick={() => { setSelCat("all"); setSelSub("all"); }} style={{ display: "flex", alignItems: "center", width: "100%", padding: "6px 10px", borderRadius: R, border: "none", background: selCat === "all" ? "#e8e8e8" : "transparent", cursor: "pointer", gap: 8, marginBottom: 2 }}>
            ...
            {cats.map((cat) => <CatSideItem key={cat.id} cat={cat} rests={rests} selCat={selCat} selSub={selSub} onCat={(id) => { setSelCat(id); setSelSub("all"); }} onSub={setSelSub} />)}
          </div>
        </div>

        {/* 가운데 패널 */}
        <div className="center-panel" style={{ background: C.card, borderRadius: R, display: "flex", flexDirection: "column", height: "calc(100vh - 80px)", overflow: "hidden" }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "18px 20px", flexShrink: 0 }}>
            <span style={{ fontWeight: 400, fontSize: 14, color: C.ink }}>{listTitle}</span>
            <button onClick={() => { setRForm({ name: "", cat: cats[0]?.id || "", sub: "", mapUrl: "" }); setModal("addRest"); }} style={{ display: "flex", alignItems: "center", gap: 6, background: C.ink, color: "#fff", border: "none", borderRadius: R, padding: "8px 16px", fontSize: 13, fontWeight: 500, cursor: "pointer" }}><IcPlus /> 맛집 추가</button>
          </div>
          <div style={{ overflowY: "auto", flex: 1, padding: "2px 8px" }}>
            {filtered.length === 0 && <div style={{ padding: "60px 20px", textAlign: "center", color: C.gray }}><div style={{ fontSize: 28, marginBottom: 8 }}>🍽️</div><div style={{ fontSize: 13 }}>등록된 맛집이 없어요</div></div>}
            {filtered.map((r) => {
              ...
              return (
                <div key={r.id}>
                  {/* ⭐ 맛집 변경 시 리뷰 폼 리셋 기능 추가 */}
                  <div className="row-hover" onClick={() => {
                    setSelId(isSel ? null : r.id);
                    setEditingRevId(null);
                    setRevForm({ text: "", author: "", rating: 5 });
                  }} style={{ display: "flex", alignItems: "center", gap: 14, padding: "12px 12px", borderRadius: R, cursor: "pointer", background: isSel ? "#f8ffd8" : "transparent" }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                        ...
                        <span style={{ fontSize: 12, color: C.muted }}>{ar || "—"}</span>
                        <span style={{ fontSize: 11.5, color: C.gray }}>({r.reviews?.length || 0})</span>
                        {r.sub && <span style={{ fontSize: 10, color: C.muted, border: `1px solid ${C.gray}`, borderRadius: 5, padding: "1px 4px", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{r.sub}</span>}
                      </div>
                    </div>
          ...
          {selected ? (
            <div style={{ background: C.card, borderRadius: R, overflow: "hidden", animation: "fu .2s ease", maxHeight: "calc(100vh - 80px)", display: "flex", flexDirection: "column" }}>
              ...
              <div style={{ overflowY: "auto", flex: 1, padding: "20px 20px" }}>
                <div style={{ fontSize: 12, fontWeight: 500, color: C.muted, marginBottom: 15 }}>리뷰</div>
                    ...
                    <div key={rv.id || i} style={{ background: editingRevId === rv.id ? "#f8ffd8" : "#fff", border: `1px solid ${editingRevId === rv.id ? C.point : C.border}`, borderRadius: R, padding: "11px 14px", transition: "background 0.2s" }}>
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 7 }}>
                        ...
                        <div style={{ display: "flex", gap: 6 }}>
                          <button onClick={() => {
                            setEditingRevId(rv.id);
                            setRevForm({ text: rv.text, author: rv.author, rating: rv.rating || 5 });
                          }} style={{ background: "none", border: "none", cursor: "pointer", color: C.gray, fontSize: 12 }}>수정</button>
                      ...
                      <div style={{ fontSize: 13, color: C.ink, lineHeight: 1.5, fontWeight: 300, whiteSpace: "pre-wrap" }}>{rv.text}</div>
                    </div>
                  ))}
                </div>

                {/* ⭐ 리뷰 작성 폼 (수정 모드에 따라 텍스트 변경) */}
                <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: R, padding: "10px" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 15 }}>
                    <div style={{ fontSize: 12, fontWeight: 500, color: editingRevId ? C.ink : C.muted }}>
                      {editingRevId ? "✏️ 리뷰 수정하기" : "리뷰 남기기"}
                    </div>
                    {editingRevId && (
                      <button onClick={() => { setEditingRevId(null); setRevForm({ text: "", author: "", rating: 5 }); }} style={{ background: "none", border: "none", color: C.gray, cursor: "pointer", fontSize: 11 }}>
                        수정 취소
                      </button>
                    )}
                  </div>
                  ...
                  <div style={{ display: "flex", gap: 7 }}>
                    <textarea placeholder="리뷰를 입력하세요" value={revForm.text} onChange={(e) => setRevForm((p) => ({ ...p, text: e.target.value }))} onKeyDown={(e) => e.key === "Enter" && e.ctrlKey && saveReview()} rows={3} style={{ ...inp(), flex: 1, fontSize: 13, padding: "8px 10px", resize: "none", lineHeight: 1.5 }} />
                    <button onClick={saveReview} style={{ background: editingRevId ? C.point : C.ink, color: editingRevId ? C.ink : "#fff", border: "none", borderRadius: R, padding: "0 14px", fontWeight: 600, fontSize: 13, cursor: "pointer", alignSelf: "flex-start", height: 35, transition: "background 0.2s" }}>
                      {editingRevId ? "수정완료" : "등록"}
                    </button>
                  </div>
                </div>
              </div>
      ...
      {/* 모달들 */}
      {modal === "manageCats" && <ManageCatsModal cats={cats} onClose={() => setModal(null)} />}
      {(modal === "addRest" || modal === "editRest") && (
        <Modal title={modal === "addRest" ? "🏪 맛집 추가" : "✏️ 맛집 수정"} onClose={() => { setModal(null); setMd(null); }}>
      ...
      {modal === "delRest" && md && (
        <Modal title="맛집 삭제" onClose={() => { setModal(null); setMd(null); }} width={360}>
          <p style={{ fontSize: 14, color: C.ink, lineHeight: 1.7, marginBottom: 22 }}><strong style={{ fontWeight: 600 }}>{md.name}</strong>을(를) 삭제할까요?<br /><span style={{ color: C.muted, fontSize: 13 }}>후기와 별점도 함께 삭제돼요.</span></p>
          <div style={{ display: "flex", gap: 8 }}>
            <button onClick={() => delRest(md.id)} style={{ flex: 1, padding: "11px 0", border: "none", borderRadius: R, fontWeight: 600, fontSize: 13.5, cursor: "pointer", background: "#ff4040", color: "#fff" }}>삭제하기</button>
            <button onClick={() => { setModal(null); setMd(null); }} style={{ flex: 1, padding: "11px 0", border: "none", borderRadius: R, fontSize: 13.5, cursor: "pointer", background: C.bg, color: C.muted }}>취소</button>
          </div>
        </Modal>
      )}
